...
export default function TicketBuy() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        // startAt 처리 (배열형 또는 ISO 문자열형)
        if (data?.startAt) {
          let startDate;
          if (Array.isArray(data.startAt)) {
            // 배열형: [year, month, day, hour?, minute?]
            startDate = new Date(
              data.startAt[0],
              (data.startAt[1] || 1) - 1,
              data.startAt[2] || 1,
  ...
  const handleDateClick = (day) => {
    ...
    const newDate = Array.isArray(start)
      ? new Date(start[0], (start[1] || 1) - 1, day, start[3] || 0, start[4] || 0)
      : new Date(start);
  ...
  const totalPrice =
    (ticket?.basePrice || ticket?.seats?.[0]?.price || 0) +
    (ticket?.fee || 0) +
            ...
            <div className="ticket-buy-middle-box1">
              ...
              <div className="ticket-buy-day1">
                ...
                <div style={{ marginTop: "10px" }}>
                  선택 날짜: {selectedDate ? selectedDate.toLocaleString("ko-KR") : "-"}
                </div>
              </div>
              ...
              <div className="ticket-buy-day3">
                ...
                <div className="ticket-buy-seat">
                  {ticket?.seats?.map((s, idx) => (
                    <p key={idx}>
                      {s.grade}석 {s.price?.toLocaleString() || "0"}원 / {s.remaining}석
                    </p>
                  ))}
                </div>
              </div>
            ...
            <div className="ticket-set-setting">
                ...
                <div className="read-table">
                  <table>
                    <tbody>
                      ...
                      <tr>
                        <th>
                          {ticket?.startAt?.join(".")} ~ {ticket?.endAt?.join(".")}
                        </th>
                      </tr>
                      <tr>
              ...
              <table className="ticket-buy-table2">
                <tbody>
                  ...
                  <tr>
                    <th>일시</th>
                    <td>{selectedDate ? selectedDate.toLocaleString("ko-KR") : "선택 전"}</td>
                  </tr>
                  <tr>
                    <th>선택 좌석</th>
                    <td>선택 전</td>
                  ...
                  <tr>
                    <th>티켓 금액</th>
                    <td>{ticket?.basePrice?.toLocaleString() || "0"} 원</td>
                  </tr>
                  <tr>
                    <th>수수료</th>
                    <td>{ticket?.fee?.toLocaleString() || "0"} 원</td>
                  </tr>
                  <tr>
                    <th>배송료</th>
                    <td>{ticket?.deliveryFee?.toLocaleString() || "0"} 원</td>
                  </tr>
                  <tr>
                    <th>할인</th>
                    <td>{ticket?.discount?.toLocaleString() || "0"} 원</td>
                  </tr>
                  <tr>
                    <th>취소기한</th>
                    <td>{ticket?.cancelDate || "-"}</td>
                  </tr>
                  <tr>
                    <th>취소수수료</th>
                    <td>{ticket?.cancelFee ?? "-"}</td>
                  </tr>
                </tbody>
              </table>
